...
class GeometricObject {
     ...
     */
    draw (vertexAttr, colorAttr, normalAttr, modelUniform, coordFrame) {
	if(this.coordFrame !== undefined){
	    coordFrame = this.coordFrame;
	}
     ...
     * @param coordFrame
     */
    drawVectorsTo (gl, target, vertexAttr, colorAttr, normalAttr, modelUniform, coordFrame) {
	let N = this.vertices.length / 9;
	let vtx = [];
	var tmp = vec3.create();
	...
	for (let k = 0; k < N; k++) {
	    let thisPoint = vec3.fromValues(this.vertices[9*k], this.vertices[9*k+1], this.vertices[9*k+2]);
